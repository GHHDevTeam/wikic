 ...
 *  
 * 2017-05-31 3.3.0 -googlemaps3
 *                      -gesturehandlingオプションの追加。
 *                      -fullscreencontrolオプションの追加。
 *
 * 2017-10-12 3.4.0 -googlemaps3
 *                      -php7、pukiwiki1.5.1に対応した。
 *                      -keyオプションを復活させた。
 */

define ('PLUGIN_GOOGLEMAPS3_DEF_KEY', '');
...
define ('PLUGIN_GOOGLEMAPS3_DEF_GESTUREHANDLING', 'auto');   //タッチ操作での振る舞い(cooperative, greedy, none, auto)
define ('PLUGIN_GOOGLEMAPS3_DEF_FULLSCREENCONTROL', 'auto');   //フルスクリーンコントロール(none, normal, auto)
...
function plugin_googlemaps3_get_default () {
	...
	return array(
		...
		'key'              => PLUGIN_GOOGLEMAPS3_DEF_KEY,
		...
		'gesturehandling'  => PLUGIN_GOOGLEMAPS3_DEF_GESTUREHANDLING,
		'fullscreencontrol'=> PLUGIN_GOOGLEMAPS3_DEF_FULLSCREENCONTROL
	);
}
...
function plugin_googlemaps3_output($doInit, $params) {
	...
	// 初期化処理の出力
	if ($doInit) {
		$output = plugin_googlemaps3_init_output($key, $noiconname);
	} else {
		$output = "";
	}
...
EOD;
	...
	// フルスクリーンコントロール
	switch ($fullscreencontrol) {
		case 'none':
			$output .= "map_options['fullscreenControl'] = false;\n";
			break;
		case 'normal':
			$output .= "map_options['fullscreenControl'] = true;\n";
			break;
		case 'auto';
		default:
			// do nothing
...
EOD;
	...
	if ($kml) {
		...
		if (!$ismatch) {
			$query = "?plugin=attach&pcmd=open&file=".pagename_urlencode($kml)."&refer=".pagename_urlencode($page);
			$kml = $script . $query;
		}
...
function plugin_googlemaps3_init_output($key, $noiconname) {
	global $vars;
	$page = $vars['page'];
	if ($key != '') {
	    $key= "&key=".$key;
	...
	return <<<EOD
<script src="http://maps.google.com/maps/api/js?v=3.10&sensor=true&libraries=places$key" type="text/javascript" charset="UTF-8"></script>
<script type="text/javascript">
//<![CDATA[
