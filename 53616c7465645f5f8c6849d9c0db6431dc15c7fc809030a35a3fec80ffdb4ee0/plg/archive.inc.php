<?php
function plugin_archive_action()
{
    mkdir('./archive/',0777);
    mkdir('./archive/'.date("YmdHis"),0777);
    $randomid = md5(rand());
    file_put_contents('./archive/'.date("YmdHis").'/'.$randomid,file_get_html_fix($_POST['plugin_archive_url']));
    header('Location: ./archive/'.date("YmdHis").'/'.$randomid);
}
function plugin_archive_convert()
{
$html = <<<EOD
<form method="post">
    <input type="text" id="plugin_archive_url" name="plugin_archive_url" placeholder="URL"/>
    <input type="hidden" id="plugin" name="plugin" value="archive">
    <input type="submit" id="plugin_archive_submit" name="plugin_archive_submit" value="アーカイブ">
</form>
EOD;
    return $html;
}
function file_get_html_fix($url)
{
    $filedata = file_get_html($url);
