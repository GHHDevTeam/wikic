<?php
// $Id: viewedit.inc.php,v 1.01 2020/10/17 11:56:23 K Exp $

define('PLUGIN_EDIT_FREEZE_REGEX', '/^(?:#freeze(?!\w)\s*)+/im');
define('PUKIWIKI_CSS', 'skin/pukiwiki.css');

function plugin_viewedit_action()
{
    if (isset($_POST['data'])){
        ...
        $css = PUKIWIKI_CSS;
        echo "<div id=\"body\"><link rel=\"stylesheet\" type=\"text/css\" href=\"{$css}\" />" . $postdata . "</div>";
        exit;
    }
	...
	plugin_viewedit_setup_initial_pages();
	...
	if (isset($vars['preview'])) {
		return plugin_viewedit_preview($vars['msg']);
	} else if (isset($vars['template'])) {
		return plugin_viewedit_preview_with_template();
	} else if (isset($vars['write'])) {
		return plugin_viewedit_write();
	} else if (isset($vars['cancel'])) {
		return plugin_viewedit_cancel();
	}

	$postdata = @join('', get_source($page));
	if ($postdata === '') $postdata = auto_template($page);
	$postdata = remove_author_info($postdata);
	return array('msg'=>$_title_edit, 'body'=>plugin_viewedit_form($page, $postdata));
}

/**
 * Preview with template
 */
function plugin_viewedit_preview_with_template()
{
	global $vars;
	$msg = '';
	$page = isset($vars['page']) ? $vars['page'] : '';
	...
	return plugin_viewedit_preview($msg);
}
 ...
 */
function plugin_viewedit_preview($msg)
{
	global $vars;
	...
	$body .= plugin_viewedit_form($page, $msg, $vars['digest'], FALSE);

	return array('msg'=>$_title_preview, 'body'=>$body);
}

// Inline: Show edit (or unfreeze text) link
function plugin_viewedit_inline()
{
	static $usage = '&viewedit(pagename#anchor[[,noicon],nolabel])[{label}];';
...
function plugin_viewedit_write()
{
	global $vars;
	...
	if ($digest !== $oldpagemd5) {
		...
		$retvars['body'] .= plugin_viewedit_form($page, $postdata_input, $oldpagemd5, FALSE);
		return $retvars;
	}
	...
	if ($notimeupdate > 1 && $notimestamp && ! pkwk_login($vars['pass'])) {
		...
		$retvars['body'] .= plugin_viewedit_form($page, $msg, $digest, FALSE);
		return $retvars;
	}
...
function plugin_viewedit_cancel()
{
	global $vars;
	pkwk_headers_sent();
 ...
 * Setup initial pages
 */
function plugin_viewedit_setup_initial_pages()
{
	global $autoalias;
...
function plugin_viewedit_form($page, $postdata, $digest = FALSE, $b_template = TRUE)
{
...
<div class="edit_form" id="viewedit_form">
 ...
 <form action="$script" method="post" class="_plugin_viewedit_viewedit_form" style="margin-bottom:0;right:50%">
$template
  $addtag
  <input type="hidden" name="cmd"    value="viewedit" />
...
<form id="previewpost" target="preview_viewedit" action="./?plugin=viewedit" method="post">
